...
AI использовался как pair-programming и review-инструмент.

С помощью AI были подготовлены и доработаны:

- первичный аудит проекта по требованиям тестового задания;
- структура сервисного слоя и email-контракт;
- интеграция с Brevo HTTPS API;
- обработка частичных ошибок email-провайдера;
- Pest-тесты и HTTP fakes;
- OpenAPI-спецификация и документация;
- простой Blade-фронтенд для проверки API;
- черновая структура README.

Примеры использованных промптов:

```text
Проверь Laravel API по тестовому заданию: архитектура, статусы,
rate limiting, логирование, AI fallback и email-уведомления.
```

```text
Спроектируй заменяемый email provider для Brevo HTTPS API
с timeout, retry, безопасными логами и Pest Http::fake тестами.
```

```text
Проверь полный цикл POST /api/contact:
валидация → AI → сохранение → два письма → HTTP-ответ.
```

```text
Создай простой Blade-интерфейс для формы обратной связи,
не меняя контракт существующего API.
```

После генерации вручную проверялись код, конфигурация и реальные deployment logs. Пришлось исправить:
...
- отсутствие OpenAI key и проверку AI fallback;
- неработающий rate limiter;
- запись персональных данных в request logs;
- слишком сложный frontend и API Docs;
- несоответствие README фактическим тестам и реализации.

AI-результаты не принимались без проверки: код форматировался Laravel Pint, поведение проверялось Pest-тестами, OpenAPI YAML проверялся парсером, а production-сборка выполнялась через Vite.
